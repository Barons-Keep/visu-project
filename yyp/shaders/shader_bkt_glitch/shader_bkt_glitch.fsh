...
vec4 random(float dataOffset) {
  ...
  vec2 coord = vec2(offset * NOISE_TEXTURE_SIZE, offset) + halfTexelSize;
  return texture2D(noiseTexture, coord); 
}
...
void main() {
  ...
  vec2 uv = rotated_uv_resolution(v_vTexcoord, resolution, vec2(0.5, 0.5), 0.0);
  vec3 randomValues = vec3(0.0);
  vec4 randomHiFreq = whiteNoise(uv, randomValues.xy);
